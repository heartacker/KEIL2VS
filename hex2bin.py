...
def HilinkHex2bin(PROJECT_MACRO,crc_type, path='./',is_work=False):
     ...
     for _this_hex in hex_file_list:
        _fldr = os.path.dirname(_this_hex) + '\\'
        _this_bin_file_name = os.path.basename(_this_hex)[0:-4]
       #_this_map = os.path.join(_fldr,_this_bin_file_name + ".MAP")
        _this_map = os.path.join(_fldr,'%s%s'%(_this_bin_file_name,'.MAP'))
        this_hex_array = read_hex(_this_hex)
        if this_hex_array == None:
            print('%s: Warning:occur problem when read this hex file:%s !!' % (time.strftime("%H:%M:%S"), _this_hex))
            continue
        if crc_type != 0:
            if os.path.exists(_this_map):
                codelens = get_code_lens(_this_map,crc_type)
            else:
                print('%s: Warning:can not find MAP file!!' % time.strftime("%H:%M:%S"))
                codelens = 0

        _this_bin = [0x00] * (BIN_MAX_LEN)
        
        for one_line in this_hex_array:
            addr_i = int(one_line['data_addrs'],16)
            if addr_i < CLIP_ADDR_BASE:
                continue
            if addr_i > (CLIP_ADDR_BASE + BIN_MAX_LEN):
                continue
            if int(one_line['recordtype'],16) >= 2:#int(RecordType.ExtdSegment):
                continue
            if int(one_line['recordtype'],16) == 1:#int(RecordType.EndOfFile):
                break
            addr_i-=CLIP_ADDR_BASE
            i = 0
            i_len = int(one_line["byte_count"],16)
            while i < i_len:
                bytestr = one_line["main_datas"][(i * 2):(i * 2) + 2]
                _this_bin[addr_i + i] = int(bytestr,16)
                i = i + 1

        #if defined(H16) || defined(H15)
        #det_file_arr[0x7f3e] = 0x5a;
        #det_file_arr[0x7f3f] = 0x5a;
        #endif
        if PROJECT_MACRO == "H15" or PROJECT_MACRO == "H16":
            _this_bin[0x7F3E] = 0x5A
            _this_bin[0x7F3F] = 0x5A
        if crc_type != 0:
            set_crc_value(_this_bin,codelens,crc_type)    

        ver_info = get_version(_this_bin)
        is_save_ok = save_bin(_this_bin,_fldr,_this_bin_file_name,ver_info)
        if is_save_ok:
            continue
        else:
...
def get_code_lens(map_file_path,crc_type):
    """通过MAP文件读取code的长度和大小"""
    with open(map_file_path, 'rt') as maps:
        gap_start_flag = r"C O D E   M E M O R Y"  # 寻找code memory
        gap_flag = r"**GAP**"                      #寻找**gap**
        gap_stop_flag = r"M E M O R Y  * * "
        pro_size_flag = r"Program Size"
        maptxt = maps.read()
        map_all_lines = maptxt.splitlines()
        code_lens = 0
        i = 0
        while i < len(map_all_lines):
            #print("%d\t %s"%(i,map_all_lines[i]))
            if gap_start_flag in map_all_lines[i]:
                #print("%d\t %s" % (i, map_all_lines[i]))
                i+=1
                while (not gap_stop_flag in map_all_lines[i]):                    
                    if  gap_flag in map_all_lines[i]:
                        #print("%d\t %s" % (i, map_all_lines[i]))
                        mod = re.compile("[0-9A-F]{6}H")
                        info = mod.findall(map_all_lines[i])
                        code_lens += int(info[2][:-1],16)
                    i += 1
                #else:
                #    print("%d\t %s" % (i, map_all_lines[i]))
                #    print("LOOP END")
            elif pro_size_flag in map_all_lines[i]:
                #print("%d\t %s" % (i, map_all_lines[i]))
                mode = re.compile("(?<=[t|e]=)[0-9]+")
                const_code = mode.findall(map_all_lines[i]) 
                for cc in const_code:
                    code_lens += int(cc)
            i+=1
        return code_lens
...
def crc_calc(_bin,_codelens,crc_type):
    ...
    crc_var = 0
    if crc_type == 8:
        for x in range(0,_codelens):
            crc_var = crc_table[(crc_var & 0xff) ^ _bin[x]]
        return crc_var
    elif crc_type == 16:
        wtab_no = 0
        for x in range(0,_codelens):
            wtab_no = (crc_var & 0xff) ^ (_bin[x] & 0xff)
            crc_var = (crc_var >> 8) & 0xff ^ crc_table[wtab_no]
        return crc_var
    elif crc_type == 32:
        crc_var = 0xffffffff
        for x in range(0,_codelens):
            crc_var = crc_table[(crc_var ^ _bin[x]) & 0xff] ^ (crc_var >> 8)
        crc_var = crc_var ^ 0xffffffff
        return crc_var
    else:
        return crc_var

def set_crc_value(this_bin,codelens,crc_type):
    """将crc 的值写到赋值到bin文件中"""
    crc_value = crc_calc(this_bin,codelens,crc_type)
    if crc_type == 8:
        this_bin[gl_CRC_Address + 0] = crc_value
    elif crc_type == 16:
        this_bin[gl_CRC_Address + 1] = (crc_value >> 8) & 0xff
        this_bin[gl_CRC_Address + 0] = crc_value & 0xff
    elif crc_type == 32:
        this_bin[gl_CRC_Address + 3] = (crc_value >> 24) & 0xff
        this_bin[gl_CRC_Address + 2] = (crc_value >> 16) & 0xff
        this_bin[gl_CRC_Address + 1] = (crc_value >> 8) & 0xff
        this_bin[gl_CRC_Address + 0] = crc_value & 0xff
    else:
        print("crc:%d参数有误！" % crctype)
    this_bin[gl_CODE_LENGTH_STAT_API + 1] = (codelens >> 8) & 0xff
    this_bin[gl_CODE_LENGTH_STAT_API + 0] = (codelens) & 0xff
    print("%s: Code length:\t\t0x%08x" % (time.strftime("%H:%M:%S"),codelens))
    print("%s: CRC value:\t\t0x%08x" % (time.strftime("%H:%M:%S"),crc_value))
    print("%s: CRC store address:\t0x%08x" % (time.strftime("%H:%M:%S"),gl_CRC_Address))
    print("%s: Code lens store addr: 0x%08x" % (time.strftime("%H:%M:%S"),gl_CODE_LENGTH_STAT_API))

def read_hex(filePath):
    """
    功能描述：将传入的文件路径行的hex文件生成按照hex格式分解的list
    参数：hex 文件路径
    返回值：hex分解后的list
    异常描述：文件不存在时直接返回None
    修改记录
    ...
    if not os.path.exists(fullFileName):
        ...
        with open(fullFileName, 'r') as f:
            a = f.read()
        hexArray = a.splitlines()
        hex_fmt_list = []
        for hexOneLine in hexArray:
            #print(hexOneLine)
            _hexfmt = hex_formator(hexOneLine)
            if _hexfmt is None:
                return None
            else:
                hex_fmt_list.append(_hexfmt)
                #print(_hexfmt)
        return hex_fmt_list

def save_bin(bin_array,filePath,fileName,ver,txt_nl=16):
    """
